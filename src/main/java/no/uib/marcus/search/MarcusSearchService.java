...
public class MarcusSearchService implements SearchService, Serializable {
    ...
    /**
     * Get all documents and apply a post filter.
     *
     * @param queryStr a query string, can be <code>NULL</code> which means
     * match all documents.
    ...
    private TermsBuilder getTermsAggregation(JsonObject facet) {
        ...
        //Set order
        if (facet.has("order")) {
            Order order = Order.count(false);//default to count descending
            if (facet.get("order").getAsString().equalsIgnoreCase("count_asc")) {
     ...
     * Elasticsearch. The method is in experimentation phase and maybe removed in the
     * future releases.
     * <p>
     ...
     **/
    public String toJsonString(SearchResponse response, boolean isPretty) {
        try {
